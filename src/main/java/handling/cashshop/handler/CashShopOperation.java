...
//        doCSPackets(c);
    ...
    private static void sendCashPurchaseFail(final MapleClient c, final int itemId, final int sn, final String reason) {
        StringBuilder msg = new StringBuilder("Cash Shop purchase failed. ItemId: ");
        msg.append(itemId > 0 ? String.valueOf(itemId) : "unknown");
        msg.append(", SN: ").append(sn > 0 ? String.valueOf(sn) : "unknown");
        msg.append(", Reason: ").append(reason);
        c.getSession().write(CWvsContext.broadcastMsg(1, msg.toString()));
        c.getSession().write(CWvsContext.enableActions());
    }

    private static void sendCashPurchaseFail(final MapleClient c, final CashItemInfo item, final int sn, final String reason) {
        sendCashPurchaseFail(c, item == null ? 0 : item.getId(), sn, reason);
    }

    private static void sendCashPurchaseFail(final MapleClient c, final CashItem item, final int sn, final String reason) {
        sendCashPurchaseFail(c, item == null ? 0 : item.getItemId(), sn, reason);
    }
...
//            final CashItem item = CashItemFactory.getInstance().getMenuItem(sn);
            ...
            if (item == null) {
                sendCashPurchaseFail(c, 0, sn, "Item SN was not found in cashshop_items.");
            ...
            if (itemz != null) {
                ...
                sendCashPurchaseFail(c, item, sn, "Server could not create the cash item.");
                c.getSession().write(CSPacket.sendCSFail(0));
            }
...
//            chr.modifyCSPoints(type, -toCharge, true);
            ...
            if (item != null) {
                if (chr.getCSPoints(toCharge) < item.getPrice()) {
                    sendCashPurchaseFail(c, item, sn, "Not enough cash points. Required: " + item.getPrice() + ", current: " + chr.getCSPoints(toCharge));
                    c.getSession().write(CSPacket.sendCSFail(3));
                    return;
                }
                if (!item.genderEquals(c.getPlayer().getGender())/* && c.getPlayer().getAndroid() == null*/) {
                    sendCashPurchaseFail(c, item, sn, "Gender restriction does not match the character.");
                    ...
                    sendCashPurchaseFail(c, item, sn, "This item is blocked from Cash Shop purchase.");
                    ...
                    sendCashPurchaseFail(c, item, sn, "Cash inventory is full.");
                    c.getSession().write(CSPacket.sendCSFail(0xB2));
                    return;
                }
                for (int id : GameConstants.cashBlock) {
                    if (item.getId() == id) {
                        sendCashPurchaseFail(c, item, sn, "This item is listed in GameConstants.cashBlock.");
                        c.getSession().write(CWvsContext.broadcastMsg(1, "You cannot purchase this item through cash shop."));
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    }
                ...
                if (itemz != null && itemz.getUniqueId() > 0 && itemz.getItemId() == item.getId() && itemz.getQuantity() == item.getCount()) {
                    ...
                    sendCashPurchaseFail(c, item, sn, "Server could not create the cash item, or item data did not match expected id/count.");
                    c.getSession().write(CSPacket.sendCSFail(0));
                }
            } else {
                final CashItem cashItem = CashItemFactory.getInstance().getAllItem(sn);
                if (cashItem == null) {
                    sendCashPurchaseFail(c, 0, sn, "Item SN was not found in Commodity.img or cashshop_items.");
                    c.getSession().write(CSPacket.sendCSFail(0));
                    return;
                }
                if (chr.getCSPoints(toCharge) < cashItem.getPrice()) {
                    sendCashPurchaseFail(c, cashItem, sn, "Not enough cash points. Required: " + cashItem.getPrice() + ", current: " + chr.getCSPoints(toCharge));
                    c.getSession().write(CSPacket.sendCSFail(3));
                    return;
                }
                if (cashItem.getGender() != 2 && cashItem.getGender() != c.getPlayer().getGender()) {
                    sendCashPurchaseFail(c, cashItem, sn, "Gender restriction does not match the character.");
                    c.getSession().write(CSPacket.sendCSFail(0xA7));
                    return;
                } else if (c.getPlayer().getCashInventory().getItemsSize() >= 100) {
                    sendCashPurchaseFail(c, cashItem, sn, "Cash inventory is full.");
                    c.getSession().write(CSPacket.sendCSFail(0xB2));
                    return;
                }
                for (int id : GameConstants.cashBlock) {
                    if (cashItem.getItemId() == id) {
                        sendCashPurchaseFail(c, cashItem, sn, "This item is listed in GameConstants.cashBlock.");
                        c.getSession().write(CWvsContext.broadcastMsg(1, "You cannot purchase this item through cash shop."));
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    }
                }
                Item itemz = chr.getCashInventory().toItem(cashItem);
                if (itemz != null && itemz.getUniqueId() > 0 && itemz.getItemId() == cashItem.getItemId() && itemz.getQuantity() == cashItem.getQuantity()) {
                    chr.modifyCSPoints(toCharge, -cashItem.getPrice(), false);
                    chr.getCashInventory().addToInventory(itemz);
                    c.getSession().write(CSPacket.showBoughtCSItem(itemz, cashItem.getSN(), c.getAccID()));
                } else {
                    sendCashPurchaseFail(c, cashItem, sn, "Server could not create the cash item, or item data did not match expected id/count.");
                    c.getSession().write(CSPacket.sendCSFail(0));
                }
            }
