...
public class MapleServerHandler extends IoHandlerAdapter implements MapleServerHandlerMBean {
    ...
    private static void traceSelectConnection(final IoSession session, final MapleClient client, final String stage, final String detail) {
        try {
            Object remote = session == null ? "null" : session.getRemoteAddress();
...
// <editor-fold defaultstate="collapsed" desc="Packet Log Implementation">
    ...
    public void exceptionCaught(final IoSession session, final Throwable cause) throws Exception {
        final MapleClient client = session == null ? null : (MapleClient) session.getAttribute(MapleClient.CLIENT_KEY);
        traceSelectConnection(session, client, "session-exception", cause == null ? "null" : cause.toString());
        if (cause != null) {
            FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, FileoutputUtil.getString(cause));
        }
    }

    @Override
    public void sessionOpened(final IoSession session) throws Exception {
        traceSelectConnection(session, null, "session-open", "start");
        // Start of IP checking
        final String address = session.getRemoteAddress().toString().split(":")[0];

        if (BlockedIP.contains(address)) {
            traceSelectConnection(session, null, "session-open-close", "reason=blocked-ip address=" + address);
            session.close();
            return;
        }
        ...
        if (track == null) {
            ...
            if (count >= 10) {
                ...
                traceSelectConnection(session, null, "session-open-close", "reason=ip-flood address=" + address + " count=" + count);
                session.close();
                return;
            }
        }
        ...
        if (LoginServer.isShutdown()) {
            traceSelectConnection(session, null, "session-open-close", "reason=login-shutdown ip=" + IP);
            session.close();
            return;
        }
        ...
        traceSelectConnection(session, client, "session-hello-sent", "mapleVersion=" + ServerConstants.MAPLE_VERSION + " ip=" + IP);
    ...
    public void sessionClosed(final IoSession session) throws Exception {
        ...
        traceSelectConnection(session, client, "session-closed", "start");
        System.out.println("[sessionClosed] client=" + (client != null ? (client.getPlayer() != null ? client.getPlayer().getName() : "no-player") : "null"));
    ...
    public void messageReceived(final IoSession session, final Object message) {
        if (message == null || session == null) {
            traceSelectConnection(session, null, "packet-ignored", "reason=null-message-or-session");
            return;
        }
        final byte[] packet = (byte[]) message;
        final LittleEndianAccessor slea = new LittleEndianAccessor(new ByteArrayByteStream(packet));
        if (slea.available() < 2) {
            traceSelectConnection(session, null, "packet-ignored", "reason=too-short length=" + packet.length);
            return;
        }
        final MapleClient c = (MapleClient) session.getAttribute(MapleClient.CLIENT_KEY);
        if (c == null || !c.isReceiving()) {
            traceSelectConnection(session, c, "packet-ignored", "reason=client-null-or-not-receiving length=" + packet.length);
            return;
        }
        ...
        traceSelectConnection(session, c, "packet-header", "header=" + header_num + " opcode=" + RecvPacketOpcode.nameOf(header_num) + " length=" + packet.length);
    ...
    //                c.updateSecondPassword();
     ...
     //           break;  
            ...
            case CHARACTER_CARD:
                ...
                break;
            case CLIENT_ERROR:
                traceSelectConnection(c.getSession(), c, "client-error-packet", "available=" + slea.available());
...
//                System.out.println("[CLIENT ERROR] Error Code " + errortype + " Type " + type_str + "\r\n\t[Data] Length " + data_length + " [" + SendPacketOpcode.getOpcodeName(opcodeheader) + " | " + opcodeheader + "]\r\n" + HexTool.toString(slea.read((int) slea.available())));
            ...
            case PLAYER_LOGGEDIN:
                ...
                traceSelectConnection(c.getSession(), c, "player-loggedin-packet", "playerid=" + playerid);
