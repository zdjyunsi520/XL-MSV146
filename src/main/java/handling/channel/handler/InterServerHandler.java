...
public class InterServerHandler {
    ...
    private static void traceLogin(final MapleCharacter player, final String stage) {
        if (player == null) {
            return;
        }
        ...
        Item shield = equipped == null ? null : equipped.getItem((short) -10);
        String msg = "[" + FileoutputUtil.CurrentReadable_Time() + "] " + stage
                ...
                + " map=" + player.getMapId()
                + " weapon=" + (weapon == null ? 0 : weapon.getItemId())
                + " shield=" + (shield == null ? 0 : shield.getItemId())
                + "\r\n";
...
//            c.getSession().write(CWvsContext.BuffPacket.cancelBuff());
        ...
        if (transfer == null) { // Player isn't in storage, probably isn't CC
            ...
            String remoteIp = s.substring(s.indexOf('/') + 1, s.length());
            FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, "[" + FileoutputUtil.CurrentReadable_Time() + "] channel-auth-check playerid=" + playerid + " remote=" + remoteIp + " auth=" + (ip == null ? "null" : (ip.left + "/" + ip.mid + "/" + ip.right)) + "\r\n");
            if (ip == null || !remoteIp.equals(ip.left)) {
                FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, "[" + FileoutputUtil.CurrentReadable_Time() + "] channel-auth-close playerid=" + playerid + " reason=" + (ip == null ? "missing-auth" : "ip-mismatch") + "\r\n");
                if (ip != null) {
            ...
            FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, "[" + FileoutputUtil.CurrentReadable_Time() + "] before-load-db playerid=" + playerid + " channel=" + ip.right + "\r\n");
        ...
        traceLogin(player, "before-channel-add");
        channelServer.addPlayer(player);
        traceLogin(player, "after-channel-add");
        ...
        traceLogin(player, "before-char-info");
        c.getSession().write(CField.getCharInfo(player));
        traceLogin(player, "after-char-info");
  ...
  //      c.getSession().write(JobPacket.addStolenSkill());
        ...
        traceLogin(player, "before-map-add");
        player.getMap().addPlayer(player);
        traceLogin(player, "after-map-add");
        try {
            // Start of buddylist
        ...
        traceLogin(player, "login-complete");
        } catch (Exception e) {
            FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, "[" + FileoutputUtil.CurrentReadable_Time() + "] channel-login-exception playerid=" + playerid + " " + e.toString() + "\r\n" + FileoutputUtil.getString(e) + "\r\n");
            FileoutputUtil.outputFileError(FileoutputUtil.Login_Error, e);
        }
   }
