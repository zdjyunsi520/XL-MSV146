...
//                 if (slea.available() != 18L) {
    ...
    private static void returnToCharacterSelect(MapleClient c) {
        final int world = c.getWorld();
        c.disconnect(true, false);
        // Empty CHANGE_MAP is sent by the client when leaving the game; keep the session open and return to character select.
        c.updateLoginState(MapleClient.LOGIN_LOGGEDIN, c.getSessionIPAddress());
        c.setWorld(world);
        c.getSession().write(LoginPacket.getCharList(c.getSecondPassword(), c.loadCharacters(world), c.getCharacterSlots()));
    }
