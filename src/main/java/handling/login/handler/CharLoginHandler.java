...
public class CharLoginHandler {
    ...
    private static void traceSelect(final MapleClient c, final String stage, final int charId, final String detail) {
        String msg = "[" + FileoutputUtil.CurrentReadable_Time() + "] " + stage
                + " accountId=" + (c == null ? -1 : c.getAccID())
                + " account=" + (c == null ? "null" : c.getAccountName())
                + " charId=" + charId
                + " world=" + (c == null ? -1 : c.getWorld())
                + " channel=" + (c == null ? -1 : c.getChannel())
                + " loggedIn=" + (c != null && c.isLoggedIn())
                + " loginState=" + (c == null ? -1 : c.getLoginState())
                + " remote=" + (c == null ? "null" : c.getSessionIPAddress())
                + " detail=" + detail
                + "\r\n";
        FileoutputUtil.logToFile(FileoutputUtil.SelectCharTrace_Log, msg);
    }
    ...
    public static void login(final LittleEndianAccessor slea, final MapleClient c) {
        String first = slea.readMapleAsciiString();
        String second = slea.readMapleAsciiString();
        String login = second.replace("NP12:auth06:5:0:", "");
        String pwd = first;

        String firstLogin = first.replace("NP12:auth06:5:0:", "");
        if (AutoRegister.getAccountExists(firstLogin) && !AutoRegister.getAccountExists(login)) {
            login = firstLogin;
            pwd = second;
        }
        ...
        System.out.println("[Login] account=" + login + " loginok=" + loginok + " accId=" + c.getAccID() + " ip=" + c.getSessionIPAddress());
    ...
    public static void CharlistRequest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        traceSelect(c, "charlist-request", 0, "requestedWorld=" + server + " requestedChannel=" + channel + " channelExists=" + (ChannelServer.getInstance(channel) != null) + " worldExists=" + WorldOption.isExists(server));
        if (!World.isChannelAvailable(channel, server) || !WorldOption.isExists(server)) {
            traceSelect(c, "charlist-denied", 0, "requestedWorld=" + server + " requestedChannel=" + channel + " channelAvailable=" + World.isChannelAvailable(channel, server) + " worldExists=" + WorldOption.isExists(server));
        ...
        if (chars != null && ChannelServer.getInstance(channel) != null) {
            ...
            traceSelect(c, "charlist-success", 0, "characters=" + chars.size());
            ...
            traceSelect(c, "charlist-close", 0, "charsNull=" + (chars == null) + " channelExists=" + (ChannelServer.getInstance(channel) != null));
            c.getSession().close();
        }
    }
    ...
    public static void CreateChar(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        LoginInformationProvider lip = LoginInformationProvider.getInstance();
        if (job != JobType.BeastTamer) {
            for (int idx = 0; idx < items.length; idx++) {
                if (items[idx] > -1) {
                    if (!lip.isEligibleItem(gender, index, job.type, items[idx])) {
                        int replacement = lip.getFirstEligibleItem(gender, index, job.type);
                        if (replacement > -1) {
                            System.out.println("Replaced invalid item " + items[idx] + " with " + replacement + " (gender=" + gender + " slot=" + index + " job=" + job.type + ")");
        ...
        if (job == JobType.AngelicBuster || job == JobType.Kaiser) {
            ...
            equip.removeItem((short) -11);
            equip.removeItem((short) -10);
            item = li.getEquipById(newchar.getZeroWeapon(false));
            item.setPosition((byte) -11);
            item.setGMLog("Zero Character Creation");
            equip.addFromDB(item);
            item = li.getEquipById(newchar.getZeroWeapon(true));
            item.setPosition((byte) -10);
            item.setGMLog("Zero Character Creation");
            equip.addFromDB(item);
        }
        ...
        // Set meso and token coins for new character
        newchar.setMeso(100000000);
        for (int tokenId = 4310000; tokenId <= 4310101; tokenId++) {
            newchar.getInventory(MapleInventoryType.ETC).addItem(new Item(tokenId, (byte) 0, Short.MAX_VALUE));
        }
        if (job == JobType.Zero || job == JobType.Hayato) {
            Item equippedWeapon = newchar.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
            Item equippedShield = newchar.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -10);
            FileoutputUtil.logToFile(FileoutputUtil.LoginTrace_Log, "[" + FileoutputUtil.CurrentReadable_Time() + "] create-char name=" + name
                    + " jobType=" + job.type
                    + " job=" + newchar.getJob()
                    + " level=" + newchar.getLevel()
                    + " map=" + newchar.getMapId()
                    + " weaponInput=" + weapon
                    + " shieldInput=" + shield
                    + " equippedWeapon=" + (equippedWeapon == null ? 0 : equippedWeapon.getItemId())
                    + " equippedShield=" + (equippedShield == null ? 0 : equippedShield.getItemId())
                    + "\r\n");
        }
        if (MapleCharacterUtil.canCreateChar(name, c.isGm())
    ...
    public static void Character_WithoutSecondPassword(final LittleEndianAccessor slea, final MapleClient c, final boolean haspic, final boolean view) {
        ...
        boolean loggedIn = c.isLoggedIn();
        boolean tooManyAttempts = loginFailCount(c);
        boolean needsSecondPassword = currentpw != null && (!currentpw.equals("") || haspic);
        boolean authOk = c.login_Auth(charId);
        boolean channelOk = ChannelServer.getInstance(c.getChannel()) != null;
        boolean worldOk = WorldOption.isExists(c.getWorld());
        traceSelect(c, "select-no-pic-check", charId, "haspic=" + haspic + " view=" + view + " loggedIn=" + loggedIn + " tooManyAttempts=" + tooManyAttempts + " needsSecondPassword=" + needsSecondPassword + " authOk=" + authOk + " channelOk=" + channelOk + " worldOk=" + worldOk);
        if (!loggedIn || tooManyAttempts || needsSecondPassword || !authOk || !channelOk || !worldOk) {
            traceSelect(c, "select-no-pic-close", charId, "failed validation");
            c.getSession().close();
            return;
        }
        ...
        traceSelect(c, "select-no-pic-redirect", charId, "serverIp=" + ServerConfig.interface_ + " port=" + port + " tempIP=" + c.getTempIP());
        c.getSession().write(CField.getServerIP(c, port, charId));
    }
    ...
    public static void Character_WithSecondPassword(final LittleEndianAccessor slea, final MapleClient c, final boolean view) {
        ...
        boolean loggedIn = c.isLoggedIn();
        boolean tooManyAttempts = loginFailCount(c);
        boolean hasSecondPassword = c.getSecondPassword() != null;
        boolean authOk = c.login_Auth(charId);
        boolean channelOk = ChannelServer.getInstance(c.getChannel()) != null;
        boolean worldOk = WorldOption.isExists(c.getWorld());
        traceSelect(c, "select-pic-check", charId, "view=" + view + " loggedIn=" + loggedIn + " tooManyAttempts=" + tooManyAttempts + " hasSecondPassword=" + hasSecondPassword + " authOk=" + authOk + " channelOk=" + channelOk + " worldOk=" + worldOk);
        if (!loggedIn || tooManyAttempts || !hasSecondPassword || !authOk || !channelOk || !worldOk) {
            traceSelect(c, "select-pic-close", charId, "failed validation");
            c.getSession().close();
            return;
        }
        c.updateMacs(slea.readMapleAsciiString());
        if (c.CheckSecondPassword(password) && password.length() >= 6 && password.length() <= 16 || c.isGm() || c.isLocalhost()) {
            ...
            LoginServer.putLoginAuth(charId, s.substring(s.indexOf('/') + 1, s.length()), c.getTempIP(), c.getChannel());
            c.updateLoginState(MapleClient.LOGIN_SERVER_TRANSITION, s);
            traceSelect(c, "select-pic-redirect", charId, "serverIp=" + ServerConfig.interface_ + " port=" + port + " tempIP=" + c.getTempIP());
            c.getSession().write(CField.getServerIP(c, port, charId));
        } else {
            c.getSession().write(LoginPacket.secondPwError((byte) 0x14));
        }
