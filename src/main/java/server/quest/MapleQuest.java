...
public class MapleQuest implements Serializable {
    ...
    private static MapleQuest loadQuest(ResultSet rs, PreparedStatement psr, PreparedStatement psa, PreparedStatement pss, PreparedStatement psq, PreparedStatement psi, PreparedStatement psp) throws SQLException {
        ...
        while (rse.next()) {
            ...
            if (type.equals(MapleQuestRequirementType.interval)) {
                ...
                ret.startScript = req.getStringStore();
                ret.scriptedStart = ret.startScript != null && ret.startScript.length() > 0;
            } else if (type.equals(MapleQuestRequirementType.endscript)) {
                ret.endScript = req.getStringStore();
                ret.customend = ret.endScript != null && ret.endScript.length() > 0;
    ...
    public void start(MapleCharacter c, int npc) {
        if ((autoStart || checkNPCOnMap(c, npc)) && canStart(c, npc)) {
            ...
            if (!customend) {
                ...
                NPCScriptManager.getInstance().startQuest(c.getClient(), npc, getId(), endScript);
            }
        }
    }
    ...
    public boolean hasStartScriptFile(MapleCharacter c) {
        return hasQuestScriptFile(c, startScript);
    }

    public boolean hasEndScriptFile(MapleCharacter c) {
        return hasQuestScriptFile(c, endScript);
    }

    private boolean hasQuestScriptFile(MapleCharacter c, String script) {
        return script != null && script.length() > 0 && NPCScriptManager.getInstance().hasQuestScript(c.getClient(), script);
    }
