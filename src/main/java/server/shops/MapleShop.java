...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        ...
        for (MapleShopItem si : items) {
            if (si.getReqItem() > 0) {
                System.out.println("  Item: " + si.getItemId() + " | Token: " + si.getReqItem() + " x" + si.getReqItemQ());
            }
        }
    ...
    public void buy(MapleClient c, short slot, int itemId, short quantity) {
        MapleShopItem shopItem = findBySlot(slot);
        if (shopItem != null) {
            System.out.println("[Shop Buy] NPC: " + npcId + " | ItemID: " + itemId + " | Price: " + shopItem.getPrice() + " | ReqItem(Token): " + shopItem.getReqItem() + " | ReqItemQty: " + shopItem.getReqItemQ());
        } else {
            System.out.println("[Shop Buy] NPC: " + npcId + " | ItemID: " + itemId + " | shopItem is null for slot: " + slot);
        }
