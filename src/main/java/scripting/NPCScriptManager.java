...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final boolean hasQuestScript(final MapleClient c, final int quest) {
        return hasQuestScript(c, String.valueOf(quest));
    }

    public final boolean hasQuestScript(final MapleClient c, final String script) {
        return script != null && script.length() > 0 && getInvocable("quest/" + script + ".js", c, true) != null;
    }
...
//                    System.err.println("sel "+ selection);
    ...
    public final void startQuest(final MapleClient c, final int npc, final int quest) {
        startQuest(c, npc, quest, MapleQuest.getInstance(quest).getStartScript());
    }

    public final void startQuest(final MapleClient c, final int npc, final int quest, final String script) {
        ...
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                final Invocable iv = getInvocable("quest/" + script + ".js", c, true);
                if (iv == null) {
                    dispose(c);
                    return;
                }
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, npc, quest, script, (byte) 0, iv);
                cms.put(c, cm);
    ...
    public final void endQuest(final MapleClient c, final int npc, final int quest, final boolean customEnd) {
        endQuest(c, npc, quest, customEnd, MapleQuest.getInstance(quest).getEndScript());
    }

    public final void endQuest(final MapleClient c, final int npc, final int quest, final boolean customEnd, final String script) {
        ...
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, npc, quest, script, (byte) 1, iv);
                cms.put(c, cm);
                scriptengine.put("qm", cm);
    ...
    public final void dispose(final MapleClient c) {
        ...
        if (npccm != null) {
            ...
            if (npccm.getType() == -1) {
                ...
                c.removeScriptEngine("scripts/quest/" + npccm.getScript() + ".js");
            }
        }
