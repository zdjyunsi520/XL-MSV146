...
public class MapleClient implements Serializable {
    ...
    public final void removalTask(boolean shutdown) {
        try {
            ...
            System.err.println("[removalTask] Error for player " + (player != null ? player.getName() : "null") + ": " + e.getMessage());
        }
    }
    ...
    public final void disconnect(final boolean RemoveInChannelServer, final boolean fromCS, final boolean shutdown) {
        this.sl.dump();
        if (player != null) {
            System.out.println("[Disconnect] player=" + player.getName() + " RemoveInChannel=" + RemoveInChannelServer + " fromCS=" + fromCS + " shutdown=" + shutdown);
            ...
            System.out.println("[Disconnect] Before saveToDB for " + player.getName() + " mapid=" + player.getMapId());
            player.saveToDB(true, fromCS);
            System.out.println("[Disconnect] After saveToDB for " + player.getName());
            if (shutdown) {
                player = null;
                receiving = false;
                return;
    ...
    public final int getAccountGMLevel() {
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT gm FROM accounts WHERE id = ?")) {
                ps.setInt(1, accId);
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
                    return rs.getInt("gm");
            ...
            System.err.println("[getAccountGMLevel] Error: " + e.getMessage());
        }
        return 0;
    }
