...
missing = sorted(orig_ids - trans_ids, key=int)
print(f'缺失翻译的条目数: {len(missing)}')

for mid in missing:
    pattern = rf'<imgdir name="{mid}">(.*?)</imgdir>'
    m = re.search(pattern, orig, re.DOTALL)
    if m:
        inner = m.group(1)
        strings = re.findall(r'<string name="([^"]+)" value="([^"]+)"/>', inner)
        print(f'\n{mid}:')
