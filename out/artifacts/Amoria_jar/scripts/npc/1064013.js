	...
	//	  cm.dispose();
        ...
        if (mode == 1) {
            switch (selection) {
                case 100:
                    if (cm.getParty() != null) {
                        if (cm.getDisconnected("VonBonBattle") != null) {
                            cm.getDisconnected("VonBonBattle").registerPlayer(cm.getPlayer());
                        } else if (cm.isLeader()) {
                            ...
                            while (it.hasNext()) {
										    var i = 0;
        var full = true;
        if (cm.getMap(105200510 + i).getCharactersSize() == 0 && cm.getMap(105200510 + i)) {
								            i;
            if (cm.getMap(105200510 + i).getCharactersSize())
                full = true;
				else
				full = false;
        }
            ...
            return;
                    ...
                    if (cm.getParty() != null) {
                        if (cm.getDisconnected("VonBonEasy") != null) {
                            cm.getDisconnected("VonBonEasy").registerPlayer(cm.getPlayer());
                        } else if (cm.isLeader()) {
                            var party = cm.getPlayer().getParty().getMembers();
                            var mapId = cm.getPlayer().getMapId();
                            var next = true;
                            ...
                            while (it.hasNext()) {
										    var i = 0;
        var full = true;
        if (cm.getMap(105200110 + i).getCharactersSize() == 0 && cm.getMap(105200110 + i)) {
								            i;
            if (cm.getMap(105200110 + i).getCharactersSize())
                full = true;
				else
				full = false;
        }
