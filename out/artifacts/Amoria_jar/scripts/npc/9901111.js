//script by Alcandon

var status;

function start() {
    status = -1;
    action(1, 0, 0);
}

function action(mode, type, selection) {
    if (mode == 1)
        status++;
    else {
        cm.sendOk("#e你好！#r#h#k 我是MapleBlade的枫叶*2*系统！ #r神力枫叶系统在自由市场 :)#k。这只是一个简单的交易者。你需要持有此物品：#i4001126##e#d");
        cm.dispose();
        return;
    }
		if (status == 0) {
	cm.sendSimple ("\r\n#L83##k用枫叶兑换超酷稀有物品！" +
                 "尚未开放，抱歉。 #e#d");
	  } else if (selection == 80) {
               cm.sendSimple ("\r\n#L0##g用10张蓝色许愿票兑换10只银色史莱姆！" +
			"选择你想兑换的奖品！#e#d");
	  } else if (selection == 83) {
               cm.sendSimple ("\r\n#L13##r#k用100枫叶兑换1,000,000金币" + 
            "\r\n#L1##r#k用200枫叶兑换150个万能疗伤药" + 
            "\r\n#L2##b#k用600枫叶兑换3个玛瑙苹果" + 
            "\r\n#L3##r#k用1200枫叶兑换1张蓝色许愿票" + 
            "\r\n#L5##b#k用1500枫叶兑换1张混沌卷轴" + 
            "\r\n#L6##b#k用2000枫叶兑换1张白卷轴" + 
            "\r\n#L50##b#k更多选项即将推出！ :)"+
            "做得好，这是你的150个万能疗伤药！");
} else if (selection == 1) {
		if (cm.haveItem(4001126, 200)){
		cm.gainItem(2000005, 150);
		cm.gainItem(4001126, -200);
		cm.sendOk("#e#r你的 #i4001126# 不够！！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 2) {
        if (cm.haveItem(4001126, 600)){ 
        cm.gainItem(2022179, 3); 
        cm.gainItem(4001126, -600); 
        cm.sendOk("做得好，这是你的 #i4031545#！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 3) { 
        if (cm.haveItem(4001126, 1200)){ 
        cm.gainItem(4031545, 1); 
        cm.gainItem(4001126, -1200); 
        cm.sendOk("做得好，这是你的银色副官之星！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 4) { 
        if (cm.haveItem(4001126, 1500)){ 
        cm.gainItem(1122014, 1); 
        cm.gainItem(4001126, -1500); 
        cm.sendOk("#e#r你的 #i4031545# 不够！！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 13) { 
        if (cm.haveItem(4001126, 100)){ 
        cm.gainMeso(1000000); 
        cm.gainItem(4001126, -100); 
        cm.sendOk("做得好，这是你的混沌卷轴！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 5) { 
        if (cm.haveItem(4001126, 1500)){ 
        cm.gainItem(2049100, 1); 
        cm.gainItem(4001126, -1500); 
        cm.sendOk("做得好，这是你的白卷轴！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 6) { 
        if (cm.haveItem(4001126, 2000)){ 
        cm.gainItem(2340000, 1); 
        cm.gainItem(4001126, -2000); 
        cm.sendOk("有足够的枫叶就可以选任何GM卷轴！#e#d"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    }  else if (selection == 7) { 
               cm.sendSimple ("\r\n#L100##r#k用2500 #i4001126# 兑换下衣防御卷轴" + 
            "\r\n#L101##b#k用2500 #i4001126# 兑换弓攻击卷轴" + 
            "\r\n#L102##r#k用2500 #i4001126# 兑换披风魔防卷轴" + 
            "\r\n#L103##b#k用2500 #i4001126# 兑换披风物防卷轴" + 
            "\r\n#L104##r#k用2500 #i4001126# 兑换拳套攻击卷轴" + 
            "\r\n#L105##b#k用2500 #i4001126# 兑换弩攻击卷轴" + 
            "\r\n#L106##r#k用2500 #i4001126# 兑换短剑攻击卷轴" + 
            "\r\n#L107##b#k用2500 #i4001126# 兑换手套攻击卷轴" + 
            "\r\n#L108##r#k用2500 #i4001126# 兑换手套敏捷卷轴" + 
            "\r\n#L109##b#k用2500 #i4001126# 兑换头盔防御卷轴" + 
            "\r\n#L110##r#k用2500 #i4001126# 兑换头盔体力卷轴" + 
            "\r\n#L111##b#k用2500 #i4001126# 兑换单手斧攻击卷轴" + 
            "\r\n#L112##r#k用2500 #i4001126# 兑换单手钝器攻击卷轴" + 
            "\r\n#L113##b#k用2500 #i4001126# 兑换单手剑攻击卷轴" + 
            "\r\n#L114##r#k用2500 #i4001126# 兑换全身铠甲敏捷卷轴" + 
            "\r\n#L115##b#k用2500 #i4001126# 兑换枪攻击卷轴" + 
            "\r\n#L116##r#k用2500 #i4001126# 兑换盾牌防御卷轴" + 
            "\r\n#L117##b#k用2500 #i4001126# 兑换鞋子敏捷卷轴" + 
            "\r\n#L118##r#k用2500 #i4001126# 兑换鞋子跳跃卷轴" + 
            "\r\n#L119##b#k用2500 #i4001126# 兑换鞋子速度卷轴" + 
            "\r\n#L120##r#k用2500 #i4001126# 兑换矛攻击卷轴" + 
            "\r\n#L121##b#k用2500 #i4001126# 兑换长杖魔攻卷轴" + 
            "\r\n#L122##r#k用2500 #i4001126# 兑换上衣防御卷轴" + 
            "\r\n#L123##b#k用2500 #i4001126# 兑换双手斧攻击卷轴" + 
            "\r\n#L124##r#k用2500 #i4001126# 兑换双手钝器攻击卷轴" + 
            "\r\n#L125##b#k用2500 #i4001126# 兑换双手剑攻击卷轴" + 
            "\r\n#L126##r#k用2500 #i4001126# 兑换短杖魔攻卷轴" + 
            "做得好，这是你的褐色工地手套！");
  } else if (selection == 8) { 
        if (cm.haveItem(4001126, 2500)){ 
        cm.gainItem(2082149, 1); 
        cm.gainItem(4001126, -2500); 
        cm.sendOk("做得好，这是你的水晶飞镖！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 9) { 
        if (cm.haveItem(4001126, 2500)){ 
        cm.gainItem(2070016, 1); 
        cm.gainItem(4001126, -2500); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 10) { 
        if (cm.haveItem(4001126, 3000)){ 
        cm.gainItem(1082223, 1); 
        cm.gainItem(4001126, -3000); 
        cm.sendOk("做得好，这是你的平衡之怒！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 11) { 
        if (cm.haveItem(4001126, 4000)){ 
        cm.gainItem(2070018, 1); 
        cm.gainItem(4001126, -4000); 
        cm.sendOk("做得好，这是你的扎昆头盔！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 12) { 
        if (cm.haveItem(4001126, 7500)){ 
        cm.gainItem(1002357, 1); 
        cm.gainItem(4001126, -7500); 
        cm.sendOk("做得好，这是你的GM卷轴！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 100) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2040603, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 101) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2044503, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 102) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2041024, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 103) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2041050, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 104) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2044703, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 105) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2044603, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 106) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2043303, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！")
        cm.dispose(); 
        } 
    } 
else if (selection == 107) { 
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2040807, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 108) {
        if (cm.haveItem(4001126, 400)){ 
        cm.gainItem(2040806, 1); 
        cm.gainItem(4001126, -400); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    }  
	else if (selection == 109) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040006, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 110) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040007, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 111) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2043103, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 112) {
        if (cm.haveItem(4001126, 400)){  
		cm.gainItem(2043203, 1);
		cm.gainItem(4001126, -400); 		
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 113) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2043003, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 114) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040506, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 115) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2044403, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 116) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040903, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 117) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040709, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 118) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2040710, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 119) {
        if (cm.haveItem(4001126, 400)){  
		cm.gainItem(2040711, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 120) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2044303, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 121) {
        if (cm.haveItem(4001126, 400)){  
		cm.gainItem(2043803, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 122) {
        if (cm.haveItem(4001126, 400)){  
		cm.gainItem(2040403, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 123) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2044103, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 124) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2044203, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 125) {
        if (cm.haveItem(4001126, 400)){ 
		cm.gainItem(2044003, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 126) {
        if (cm.haveItem(4001126, 400)){  
		cm.gainItem(2043703, 1);
		cm.gainItem(4001126, -400); 
		cm.sendOk("做得好，这是你的暴风手套！");
		cm.dispose();
	  } else {
		cm.sendOk("做得好，这是你的3个玛瑙苹果！")
		cm.dispose();
		}
	}
else if (selection == 10) { 
        if (cm.haveItem(4001126, 500)){ 
        cm.gainItem(1082223, 1); 
        cm.gainItem(4001126, -500); 
        cm.sendOk("你的 #i4001126# 不够！！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的3个玛瑙苹果！") 
        cm.dispose(); 
        } 
    }  
else if (selection == 11) { 
        if (cm.haveItem(4001126, 600)){ 
        cm.gainItem(2070018, 1); 
        cm.gainItem(4001126, -600); 
        cm.sendOk("做得好，这是你的扎昆头盔！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 12) { 
        if (cm.haveItem(4001126, 700)){ 
        cm.gainItem(1002357, 1); 
        cm.gainItem(4001126, -700); 
        cm.sendOk("做得好，这是你的GM卷轴！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 30) { 
        if (cm.haveItem(4001126, 30)){ 
        cm.gainItem(2041024, 1); 
        cm.gainItem(4001126, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 31) { 
        if (cm.haveItem(4001126, 30)){ 
        cm.gainItem(2041025, 1); 
        cm.gainItem(4001126, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 32) { 
        if (cm.haveItem(4001126, 30)){ 
        cm.gainItem(2044703, 1); 
        cm.gainItem(4001126, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 33) { 
        if (cm.haveItem(4001126, 30)){ 
        cm.gainItem(2044603, 1); 
        cm.gainItem(4001126, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("你的 #i4031545# 不够！！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 34) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043303, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 35) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040807, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 36) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040806, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 37) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040006, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 38) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040007, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 39) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043103, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 40) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043203, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 41) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043003, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 42) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040506, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 43) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044403, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 44) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040903, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 45) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040709, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 46) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040710, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 47) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040711, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 48) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044303, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 49) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043803, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 50) { 
cm.sendOk("更多内容即将推出 :)");
cm.dispose();
    } 
else if (selection == 51) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044103, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 52) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044203, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    } 
else if (selection == 53) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044003, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("更多内容即将推出 :)") 
        cm.dispose(); 
        } 
    }
else if (selection == 54) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043703, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 55) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043103, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 56) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043203, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 129) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2043003, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 130) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040506, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 131) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2044403, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 132) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040903, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 133) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040709, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
else if (selection == 134) { 
        if (cm.haveItem(4031545, 30)){ 
        cm.gainItem(2040710, 1); 
        cm.gainItem(4031545, -30); 
        cm.sendOk("做得好，这是你的暴风手套！"); 
        cm.dispose(); 
      } else { 
        cm.sendOk("做得好，这是你的1,000,000金币！") 
        cm.dispose(); 
        } 
    } 
}
