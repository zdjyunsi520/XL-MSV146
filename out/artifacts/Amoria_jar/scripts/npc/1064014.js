...
function action(mode, type, selection) {
	...
	var time = parseInt(data);
        cm.sendSimple("#b#L101##v03994116# 普通猩红女王#l\r\n#b#L100##v03994442# 混沌猩红女王#l");
    } else if (status == 1) {
        if (mode == 1) {
            switch (selection) {
                case 100:
                        ...
                        if (cm.getDisconnected("QueenBattle") != null) {
                            ...
                            while (it.hasNext()) {
										    var i = 0;
        var full = true;
        if (cm.getMap(105200710 + i).getCharactersSize() == 0 && cm.getMap(105200710 + i)) {
								            i;
            ...
            return;
                    ...
                    if (cm.getParty() != null) {
                        if (cm.getDisconnected("QueenEasy") != null) {
                            cm.getDisconnected("QueenEasy").registerPlayer(cm.getPlayer());
                        } else if (cm.isLeader()) {
                            var party = cm.getPlayer().getParty().getMembers();
                            var mapId = cm.getPlayer().getMapId();
                            var next = true;
                            ...
                            while (it.hasNext()) {
										    var i = 0;
        var full = true;
        if (cm.getMap(105200310 + i).getCharactersSize() == 0 && cm.getMap(105200310 + i)) {
								            i;
            if (cm.getMap(105200310 + i).getCharactersSize())
                full = true;
				else
				full = false;
        }
            ...
            return;
                            ...
                            if (next) {
                                var q = cm.getEventManager("QueenEasy");
                                if (q == null) {
                                    cm.sendOk("发生未知错误。");
									cm.dispose();
                                } else {
