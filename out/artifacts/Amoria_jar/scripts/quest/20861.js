...
function end(mode, type, selection) {
    ...
    if (status == 0) {
	qm.sendYesNo("你做出决定了吗？这个决定将是最终的，所以在决定之前请三思。你确定要成为一名圣光剑士吗？");
    } else if (status == 1) {
	qm.sendNext("我刚刚重塑了你的身体，使其完美适合圣光剑士。如果你想变得更强大，使用属性窗口（S）来提升相应的属性。如果你不确定要提升什么，只需点击#b自动#k。");
	if (qm.getJob() != 1100) {
	...
	qm.sendNextPrev("与你做初心者的时期不同，一旦你成为圣光剑士，当你HP耗尽时，你将损失一部分经验值，明白吗？");
    } else if (status == 5) {
